...
export default function EditSupply() {
  ...
  useEffect(() => {
    if (supply) {
      setFormData({
        ...
        minimum_quantity: supply.minimum_quantity || "1",
  ...
  const updateMutation = useMutation({
    mutationFn: (supplyData) => base44.entities.Supply.update(supplyId, supplyData),
    onSuccess: () => {
      ...
      navigate(createPageUrl(`SupplyDetail?id=${supplyId}`));
    },
    onError: (error) => {
  ...
  const handleSubmit = (e) => {
    ...
    const cleanedData = Object.fromEntries(
      Object.entries(formData)
        .map(([key, value]) => [
          key, 
          ["quantity", "minimum_quantity", "cost"].includes(key) && value !== "" ? Number(value) : value
        ])
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center gap-4 mb-8">
        <button
          onClick={() => navigate(createPageUrl(`SupplyDetail?id=${supplyId}`))}
        ...
        <div>
          ...
          <p style={{ color: "var(--text-secondary)" }}>Update {supply?.name}'s information</p>
        </div>
      </div>
